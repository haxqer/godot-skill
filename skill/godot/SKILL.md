---
name: godot
description: Godot 4.x project development skill (verified on 4.7) — inspect and edit projects, scenes, UI, resources, and project settings headlessly; author tilesets and ASCII-painted TileMapLayer/GridMap levels, themes, sprite-sheet and keyframe animation, audio buses, custom Resource data, collision/CSG/navmesh bakes, glTF export, shader globals, and multiplayer replication; lint GDScript and scenes without Godot (Godot 3 to 4 renames, un-inferable `:=`, dead `$Path`/`%Name`/signal targets, missing files); look up any operation with `help`; copy verified GDScript templates and follow step-by-step playbooks; run, debug, unit-test, and export; verify everything as text — node-tree dumps, UI-layout reports, ASCII level and image read-backs, screenshot summaries with pass/fail gates — so a model with no vision can still confirm what it built. Use when Codex needs to build or fix a Godot game, write or repair .gd/.tscn/.tres files, wire scripts and signals, paint levels, theme menus and HUDs, add art/music/story content, migrate Godot 3 code, validate or export builds, or check a running scene without screenshots. Prefer host-native Godot tools when exposed and use the bundled portable workflows otherwise.
---
...
| --- | --- | --- |
| New 2D pixel-art project | `references/playbooks.md` §1 | `validate_project.py --pretty` → `"ok": true` |
| Player controller (2D/3D) | `templates/gdscript/player_*.gd`, playbooks §2, §3, §12 | `run_scenario.py` → `"ok": true` |
| Enemy, damage, health | `templates/gdscript/{enemy_patrol_2d,health,hitbox,hurtbox}.gd`, playbook §4 | `run_scenario.py` → `"ok": true` |
...
| HUD, pause, dialog | `templates/gdscript/{hud,pause_menu,dialog_box}.gd`, playbooks §7–§9 | `run_scenario.py` `ui_report` → `findings=0` |
| Save / load | `templates/gdscript/save_manager.gd`, playbook §10 | `run_project.py` + `godot_log_parser.py` |
| Scene fades, music, SFX | `templates/gdscript/{scene_transition,audio_manager}.gd`, playbook §11 | `validate_project.py` → `counts.warnings == 0` |
| Writing or editing any `.gd` | `references/gdscript_conventions.md`, then `lint_project.py` | `lint_project.py` → `counts.errors == 0` |
| Project written for Godot 3 | `references/godot3_to_4.md` | `lint_project.py --only godot3_api` → `counts.errors == 0` |
...
| Forgot an op's parameters | dispatcher `help '{"op":"add_node"}'` | the printed `command` runs as-is |
| Runtime errors or a project that will not boot | `references/debugging.md` | `run_project.py` → `"ok": true` |
| Verifying anything without vision | `references/automation_api.md` "Verification Without Vision" | `dump_tree` / `ui_report` / `inspect_image` / `inspect_tilemap` |
| Packaging a build | `references/export_targets.md` | `export_project.py --preflight-only` |
...
- Read `references/playbooks.md` when the task matches a common shape (new project, player, enemy, tile level, menu, HUD, pause, dialog, save/load, transitions, 3D starter). Each playbook is a numbered list of runnable command blocks plus a Verify block with the exact expected result — follow it literally rather than improvising an order.
- Run `python3 /absolute/path/to/godot/scripts/debug/lint_project.py /absolute/path/to/project --pretty` before and after touching any `.gd`, `.tscn`, or `.tres`. It needs no Godot, finishes in well under a second, and reports what Godot's own messages hide: Godot 3 API and syntax with the exact 4.x replacement, `:=` reads that will not parse, `$Path`/`%Name` references to nodes that do not exist in the scene the script is attached to (with the real children listed), `%Name` without `unique_name_in_owner`, `[connection]` targets whose method is missing, and `ext_resource`/autoload paths that are not on disk. `error` means Godot will refuse to parse or the reference cannot resolve; `warning` means it compiles but is risky.
...
- Run `help '{}'` through the dispatcher before guessing an operation or parameter name; `help '{"op":"add_node"}'` prints the parameters (`(required)`/`(default: X)` plus value shapes), a runnable example, the gotchas, and a ready-to-paste command.
...
7. For music and SFX, prefer integrating provided or separately generated audio through the project's existing import settings, audio buses, and player nodes instead of inventing a new audio pipeline.
